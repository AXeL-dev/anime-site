...
function onClickPrev() {
	if (currentImage == 0) {
		slideTo(imageNumber - 1); // -1 car on a 1 image dans chaque block du slider
	}
	else {
...
function closeFullMode(childToRemove) {
	...
	document.body.className = 'body_image_style';
...
/*--------- jeu-de-devinette --------*/
function randomNumber(rMin, rMax) { // fonction qui génère un nombre aléatoire entre rMin et rMax (@! si rMin > 0 => rMax doit être = à rMax - 1 (plutot => à rMax - rMin))
